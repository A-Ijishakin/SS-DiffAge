...
# from templates import BrainAge_autoenc_130M 
from utils import load_img  

# wandb.login(key='97a9261aafe47dfa064544eb8264b1e4f992f2af')
# wandb.init(project='BrainAge') 
...
class LitModel(pl.LightningModule):
    def __init__(self, conf: TrainConfig):
        ...
        transforms_list = [transforms.ToPILImage(), transforms.Resize((128, 128)),
                    transforms.ToTensor()]

        # #input array to look at the computational graph 
        # self.example_input_array = transforms.Compose(transforms_list)(load_img('datasets/BrainAge/train/data/%09d.nii' % (51)).astype(np.float32)).reshape(1, 1, 128, 128)
    ...
    def training_epoch_end(self, outputs, gen= False) -> None:
        ...
        if self.current_epoch > self.conf.load_in: 
            batch_size = 64
            train_loader = torch.utils.data.DataLoader(BrainAgeData(train = True, val = False, 
                                test = False, epoch_end=True), shuffle = False, batch_size=batch_size) 
            #3374
            val_loader = torch.utils.data.DataLoader(BrainAgeData(val = True), shuffle = False, batch_size=batch_size) 
            ...
            # net = net.to('cuda') 
            # net.eval() 
            # encoder = self.model.encoder.to('cuda') 
            # encoder.eval() 
            ...
            with torch.no_grad():
                for loader in loaders: 
                    ...
                    print(f'{mode} MAE: {mae}, R: {r}, p: {p}, R2: {r2}') 
                        ...
                        #     fig.savefig(os.path.join('proto_images', 'fine_tune_scatter.png'))
                ...
                # self.logger.experiment.add_scalar('L1 Norm', torch.norm(net.weight, p=1), 
                #                                     self.current_epoch) 

            # # self.model = self.model.to('cuda') 

            if not os.path.isdir('./net_checkpoints'): 
                os.mkdir('./net_checkpoints') 
                # Save the model

        # torch.save(self.model.network.state_dict(), f'./net_checkpoints/epoch_{self.current_epoch}.ckpt') 

        # #average the prototypes over the last 6 epochs 
        # if self.current_epoch % 6 == 0 and self.current_epoch > 236: 
        #     # Define a list to store the weights of the linear layers
        #     weights = {f'check{idx}: []' for idx in range(6)}

        #     for i in range(6):
        #         # Load the checkpoint 
        #         model = torch.load(f'./net_checkpoints/epoch_{self.current_epoch - i}.ckpt') 
        #         self.model.network.load_state_dict(model) 
        #         current_net = self.model.network 
        #         # Loop through the model's modules and append the weights of the linear layers to the list
        #         for module in current_net.modules():
        #             if isinstance(module, nn.Linear):
        #                 weights[f'check{i}'].append(module.weight.data.clone())

        #     # Take the average of the weights from the 6 checkpoints
        #     average_weights = torch.stack(weights).mean(dim=0)

        #     # Update the weights of the linear layers in the model with the averaged weights
        #     i = 0
        #     for module in self.model.network.modules():
        #         if isinstance(module, nn.Linear):
        #             module.weight.data = average_weights[i:i+module.weight.data.shape[0], :]
        #             i += module.weight.data.shape[0]
            ...
            if self.conf.train_mode == TrainMode.diffusion:
                ...
                if self.conf.Proto:
                  losses = self.sampler.training_losses(model=self.model,
                                      ...
                                      alpha=self.conf.alpha, _lambda=self.conf._lambda, beta = self.conf.beta,  
                                      network = self.model.network, epsilon=self.conf.epsilon, 
                                      KL=False, epoch=self.current_epoch, load_in=self.conf.load_in) 
