...
class GaussianDiffusionBeatGans:
    ...
    def training_losses(self,
        ...
        labels=None,
        Proto=False,
        alpha=0, _lambda=0, beta=0,
        prototype_vectors=None,
        network =None, epsilon = None, KL = None, option1=False, epoch=0, updater=None, 
        ...
        if self.loss_type in [
            ...
            #get the indices of all of the images in the batch with a label which is bigger than 18 (to prevent bad data from interfering)
            indices = torch.tensor(np.array([idx for (idx, label) in enumerate(list(to_cpu(labels))) if label >= 18])).to('cuda') 

            #get the good data and labels
            x_semi = torch.index_select(x_start, 0, indices)
            labels = torch.index_select(labels, 0, indices) 
        
            #embed the data 
            embeddings = model.encoder(x_semi)  
